...
Usage:
    silvergrain input.jpg output.jpg --grain-radius 0.12 --samples 200
    silvergrain input.jpg output.jpg --mode luminance --grain-radius 0.15
"""
...
def render_luminance_mode(pil_image: Image.Image, renderer: FilmGrainRenderer) -> Image.Image:
    ...
    This matches the C++ code's "black-and-white" mode behavior.
    """
...
def render_rgb_mode(pil_image: Image.Image, renderer: FilmGrainRenderer) -> Image.Image:
    ...
    This matches the C++ code's "color" mode behavior.
    """
    img_array = np.array(pil_image, dtype=np.float32) / 255.0
